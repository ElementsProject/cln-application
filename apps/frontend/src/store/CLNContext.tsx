...
import { ApplicationActions, SATS_MSAT } from '../utilities/constants';
import { BkprTransaction, Fund, FundChannel, FundOutput, Invoice, ListInvoices, ListPayments, ListOffers, ListPeers, NodeInfo, Payment, ListPeerChannels, ListNodes, Node, NodeFeeRate } from '../types/lightning-wallet.type';
...
const defaultCLNState: CLNState = {
...
const clnReducer = (state, action) => {
  logger.info('CLN State: ', state);
  switch (action.type) {
    ...
    case ApplicationActions.SET_LIST_FUNDS:
      ...
      return { ...state, walletBalances: { ...balances, isLoading: false, error: action.payload.error }, listFunds: action.payload };
    case ApplicationActions.SET_LIST_PEERS:
      return { ...state, listPeers: action.payload };
    case ApplicationActions.SET_LIST_CHANNELS:
      ...
      const aggrChannels = aggregatePeerChannels(listPeerChannels, listNodes.nodes);
      return { ...state, listChannels: { ...aggrChannels, isLoading: false, error: action.payload.error } };
    case ApplicationActions.SET_LIST_INVOICES:
      ...
      if (!state.listPayments.isLoading) {
        ...
        return { ...state, listLightningTransactions: { isLoading: false, error: action.payload.error, clnTransactions: mergedTransactions }, listInvoices: { ...action.payload, invoices: sortedInvoices } };
      }
      return { ...state, listInvoices: { ...action.payload, invoices: sortedInvoices } };
    case ApplicationActions.SET_LIST_SEND_PAYS:
      const sortedPayments = action.payload.payments?.sort((p1, p2) => ((p1.created_at && p2.created_at && p1.created_at > p2.created_at) ? -1 : 1));
      ...
      if (!state.listInvoices.isLoading) {
        ...
        return { ...state, listLightningTransactions: { isLoading: false, error: action.payload.error, clnTransactions: mergedTransactions }, listPayments: { ...action.payload, payments: groupedMPPs } };
      }
      return { ...state, listPayments: { ...action.payload, payments: sortedPayments } };
    ...
    case ApplicationActions.SET_LIST_BITCOIN_TRANSACTIONS:
      const sortedTransactions = action.payload.events?.sort((t1, t2) => ((t1.timestamp && t2.timestamp && t1.timestamp > t2.timestamp) ? -1 : 1));
      const filteredTransactions = filterOnChainTransactions(sortedTransactions, state.nodeInfo.version);
      return { ...state, listBitcoinTransactions: { isLoading: false, error: action.payload.error, btcTransactions: filteredTransactions } };
...
const CLNContext = React.createContext({
  ...defaultCLNState,
  setNodeInfo: (info: NodeInfo) => { },
  setListFunds: (fund: Fund) => { },
  setListPeers: (list: ListPeers) => { },
  setListChannels: (data: (ListPeerChannels | ListNodes)) => { },
  setListInvoices: (list: ListInvoices) => { },
  setListPayments: (list: ListPayments) => { },
  setListOffers: (list: ListOffers) => { },
  setListBitcoinTransactions: (list: any) => { },
  setFeeRate: (feeRate: NodeFeeRate) => { },
  clearStore: () => { },
});

const CLNProvider: React.PropsWithChildren<any> = (props) => {
  const [clnState, dispatchCLNAction] = useReducer(clnReducer, defaultCLNState);

  const setNodeInfoHandler = (info: NodeInfo) => {
    dispatchCLNAction({ type: ApplicationActions.SET_NODE_INFO, payload: info });
  };

  const setListFundsHandler = (fund: Fund) => {
    dispatchCLNAction({ type: ApplicationActions.SET_LIST_FUNDS, payload: fund });
  };

  const setListPeersHandler = (list: ListPeers) => {
    dispatchCLNAction({ type: ApplicationActions.SET_LIST_PEERS, payload: list });
  };

  const setListChannelsHandler = (data: (ListPeerChannels | ListNodes)) => {
    dispatchCLNAction({ type: ApplicationActions.SET_LIST_CHANNELS, payload: { data } });
  };

  const setListInvoicesHandler = (list: ListInvoices) => {
    dispatchCLNAction({ type: ApplicationActions.SET_LIST_INVOICES, payload: list });
  };

  const setListPaymentsHandler = (list: ListPayments) => {
    dispatchCLNAction({ type: ApplicationActions.SET_LIST_SEND_PAYS, payload: list });
  };

  const setListOffersHandler = (list: ListOffers) => {
    dispatchCLNAction({ type: ApplicationActions.SET_LIST_OFFERS, payload: list });
  };

  const setListBitcoinTransactionsHandler = (list: any) => {
    dispatchCLNAction({ type: ApplicationActions.SET_LIST_BITCOIN_TRANSACTIONS, payload: list });
  };

  const setFeeRateHandler = (feeRate: NodeFeeRate) => {
    dispatchCLNAction({ type: ApplicationActions.SET_FEE_RATE, payload: feeRate });
  };

  const clearContextHandler = () => {
    dispatchCLNAction({ type: ApplicationActions.CLEAR_CONTEXT });
  };
  ...
  return <CLNContext.Provider value={clnContext}>{props.children}</CLNContext.Provider>;
};

export { CLNProvider, CLNContext };
