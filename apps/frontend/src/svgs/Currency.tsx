import React from 'react';

export const CurrencySVG = props => {
  const krSVG = (
    <svg xmlns='http://www.w3.org/2000/svg' className={`${props.svgClassName} pb-1`} width='12' height='17' viewBox='0 0 395.099 395.1'>
      <g>
        <g>
          <path className={props.className} d='M100.829,256.539l105.845-132.386c1.915-2.397,2.234-5.677,0.798-8.352c-1.378-2.568-4.043-4.158-6.975-4.158H154.88
            c-2.423,0-4.741,1.12-6.201,2.988L61.28,225.567V7.726C61.28,3.464,57.751,0,53.418,0H16.68c-4.335,0-7.864,3.47-7.864,7.726
            v379.648c0,4.262,3.529,7.726,7.864,7.726h36.738c4.333,0,7.861-3.47,7.861-7.726v-94.602l3.21-0.054l95.981,99.981
            c1.478,1.524,3.553,2.399,5.689,2.399h43.503c2.991,0,5.781-1.72,7.093-4.386c1.372-2.826,0.887-6.159-1.253-8.5L100.829,256.539z
            '/>
          <path className={props.className} d='M382.341,111.055c-52.506,0-78.271,23.495-88.757,37.055v-32.214c0-2.172-1.761-3.939-3.937-3.939h-43.195
            c-2.164,0-3.931,1.768-3.931,3.939l-0.042,275.256c0,1.052,0.426,2.068,1.147,2.771c0.727,0.745,1.749,1.177,2.802,1.177h49.473
            c2.181,0,3.954-1.768,3.954-3.942V244.422c0-8.274,1.117-16.077,2.205-22.603c6.644-36.632,30.292-58.989,63.21-59.808h17.064
            c1.046,0,2.074-0.422,2.777-1.15c0.757-0.744,1.171-1.737,1.171-2.79v-43.066C386.295,112.822,384.522,111.055,382.341,111.055z'
            />
        </g>
      </g>
    </svg>
  );

  const currencies = { 
    PLN: 
      <svg version='1.0' xmlns='http://www.w3.org/2000/svg'
      width='12' height='17' viewBox='0 0 154.000000 169.000000'
      className={props.svgClassName}
      preserveAspectRatio='xMidYMid meet'>
        <g transform='translate(0.000000,169.000000) scale(0.100000,-0.100000)'
        className={props.className} stroke='none'>
          <path d='M1070 1225 l0 -324 -122 -93 c-68 -50 -126 -96 -130 -102 -13 -18 -9
          -54 8 -70 27 -28 59 -17 147 49 48 36 89 65 92 65 3 0 5 -135 5 -300 l0 -300
          100 0 100 0 0 379 0 379 116 86 c63 48 118 94 121 103 3 8 2 27 -3 42 -7 20
          -17 27 -41 29 -26 3 -46 -8 -106 -52 -41 -31 -77 -56 -80 -56 -4 0 -7 110 -7
          245 l0 245 -100 0 -100 0 0 -325z'/>
          <path d='M114 1107 c-3 -8 -4 -47 -2 -88 l3 -74 233 -3 c127 -1 232 -5 232 -8
          0 -3 -20 -29 -44 -57 -24 -29 -139 -176 -255 -327 l-211 -275 0 -62 0 -63 395
          0 395 0 0 85 0 85 -256 0 c-141 0 -254 3 -252 8 2 4 33 45 70 92 37 47 150
          193 252 325 l185 240 1 55 c0 31 -5 61 -12 68 -19 19 -727 17 -734 -1z'/>
        </g>
      </svg>,
    BRL:
      <svg width='12' height='17' className={props.svgClassName} xmlns='http://www.w3.org/2000/svg' viewBox='0 0 122.88 98'>
        <path className={props.className} d='M87.71,88.84l-1-.06q-3.37-.21-6.65-.64t-6.27-1q-3-.53-5.79-1.28V69.37q3.64.33,7.88.54c2.83.14,5.68.25,8.58.32s5.59.11,8.1.11a27,27,0,0,0,5.89-.54A7.12,7.12,0,0,0,102,68.09a4.34,4.34,0,0,0,1.18-3.22V63.58a4,4,0,0,0-1.66-3.53,6.56,6.56,0,0,0-3.91-1.18H91.89Q79.45,58.87,73,53.4T66.59,35.06V31.52q0-11.79,7.07-17.63,5.3-4.36,14-5.47V0H99.29V8.18c1.45.08,2.87.2,4.24.34Q108,9,111.89,9.7t7,1.45V27.66c-3.29-.28-7-.51-11.1-.69s-7.84-.27-11.2-.27a29.76,29.76,0,0,0-5.36.43,6.74,6.74,0,0,0-3.65,1.71,5.09,5.09,0,0,0-1.28,3.76v1.07A4.94,4.94,0,0,0,88,37.74a8.26,8.26,0,0,0,5.26,1.4h7.07q7.52,0,12.49,2.84a18.54,18.54,0,0,1,7.51,7.72,23.82,23.82,0,0,1,2.52,11.1v3.54q0,10.18-3.54,15.54a17.35,17.35,0,0,1-10.08,7.24,50,50,0,0,1-10,1.68V98H87.71V88.84ZM0,88.35V8.69H34A34.19,34.19,0,0,1,43.64,10a18.44,18.44,0,0,1,7.61,4.34,20.08,20.08,0,0,1,5,8.21,40.4,40.4,0,0,1,1.77,13,37.9,37.9,0,0,1-.86,8.58,23.06,23.06,0,0,1-2.3,6.17,16.25,16.25,0,0,1-3.44,4.23,26.38,26.38,0,0,1-4.23,3L61,88.35H39.89L28.74,60.9H20.8V88.35Zm20.8-44h9.44a8.58,8.58,0,0,0,3-.48,5.24,5.24,0,0,0,2.2-1.56,6.72,6.72,0,0,0,1.34-2.73,16.6,16.6,0,0,0,.43-4,15.94,15.94,0,0,0-.43-4,5.14,5.14,0,0,0-3.54-3.86,9.69,9.69,0,0,0-3-.43H20.8V44.39Z'/>
      </svg>,
    CHF: 
      <svg xmlns='http://www.w3.org/2000/svg' width='10' height='9' className={props.svgClassName} viewBox='0 0 93.18 122.88'>
        <path className={props.className} fillRule='evenodd' d='M14.25,0H91.1a2.09,2.09,0,0,1,2.08,2.08v19A2.09,2.09,0,0,1,91.1,23.2H36.75V46H77.59a2.09,2.09,0,0,1,2.07,2.08v19a2.1,2.1,0,0,1-2.07,2.08H36.75V87.24H49.12a2.05,2.05,0,0,1,2,2V103.9a2.06,2.06,0,0,1-2,2H36.75V120.8a2.1,2.1,0,0,1-2.08,2.08H16.33a2.09,2.09,0,0,1-2.08-2.08V105.93H2a2,2,0,0,1-2-2V89.27a2,2,0,0,1,2-2H14.25V0Z'/>
      </svg>,
    CZK: 
      <svg version='1.0' xmlns='http://www.w3.org/2000/svg'
      width='14' height='17' viewBox='0 0 137.000000 118.000000'
      className={props.svgClassName}
      preserveAspectRatio='xMidYMid meet'>
        <g transform='translate(0.000000,118.000000) scale(0.100000,-0.100000)'
        className={props.className} stroke='none'>
          <path d='M80 600 l0 -410 50 0 50 0 0 138 0 138 69 68 69 69 46 -64 c25 -35
          91 -128 146 -206 l101 -143 71 0 70 0 -24 33 c-99 130 -328 446 -328 451 0 4
          75 81 168 171 l167 164 -70 1 -70 0 -200 -200 c-110 -110 -203 -200 -207 -200
          -5 0 -8 90 -8 200 l0 200 -50 0 -50 0 0 -410z'/>
          <path d='M946 935 l56 -75 57 0 57 0 57 70 c31 38 57 72 57 75 0 3 -24 5 -52
          5 -52 0 -54 -1 -82 -46 l-30 -47 -23 29 c-13 16 -26 37 -29 47 -4 14 -17 17
          -65 17 l-59 0 56 -75z'/>
          <path d='M939 762 c-99 -51 -148 -164 -136 -315 12 -168 114 -267 274 -267 60
          0 139 36 175 78 32 39 64 120 52 132 -5 4 -26 10 -47 12 -37 3 -40 1 -52 -35
          -55 -160 -254 -136 -295 34 -29 121 10 258 84 293 78 37 173 9 197 -58 12 -32
          17 -36 49 -36 54 0 65 11 51 51 -32 93 -106 139 -223 139 -63 0 -84 -5 -129
          -28z'/>
        </g>
      </svg>,
    DKK: krSVG,
    IDR: 
      <svg xmlns='http://www.w3.org/2000/svg' className={props.svgClassName} width='14' height='17' viewBox='0 0 122.88 105.17'>
        <path className={props.className} d='M67.5,105.17V21.89H85.1l3.22,5.26a51.88,51.88,0,0,1,4.19-3.22A17.93,17.93,0,0,1,97,21.73a17.36,17.36,0,0,1,5.53-.8,21,21,0,0,1,9,1.82,15.8,15.8,0,0,1,6.39,5.53,26.77,26.77,0,0,1,3.76,9.39A63.57,63.57,0,0,1,122.88,51a65.14,65.14,0,0,1-1.66,15.67A18.86,18.86,0,0,1,115.15,77q-4.4,3.64-12.66,3.65A43.84,43.84,0,0,1,95.08,80a26.88,26.88,0,0,1-6.76-2v27.26ZM95.41,63.53a6,6,0,0,0,3-.91A6.76,6.76,0,0,0,101,59a24,24,0,0,0,1-8.05,35.62,35.62,0,0,0-.75-8.32,7,7,0,0,0-2.2-4,5.87,5.87,0,0,0-3.6-1.08,12.1,12.1,0,0,0-4.18.7,13,13,0,0,0-3,1.45V61.92a8.49,8.49,0,0,0,3.06,1.18,18.83,18.83,0,0,0,4,.43ZM20.82,35.74h9.44a8.37,8.37,0,0,0,3-.49,5.3,5.3,0,0,0,2.2-1.55A6.83,6.83,0,0,0,36.81,31a16.12,16.12,0,0,0,.43-4,15.43,15.43,0,0,0-.43-4,5.5,5.5,0,0,0-1.34-2.53,5.68,5.68,0,0,0-2.2-1.34,9.71,9.71,0,0,0-3-.43H20.82V35.74ZM0,79.74V0H34a34.25,34.25,0,0,1,9.66,1.29A18.43,18.43,0,0,1,51.3,5.63a20.18,20.18,0,0,1,5,8.21,40.57,40.57,0,0,1,1.77,13,38.06,38.06,0,0,1-.86,8.59,22.85,22.85,0,0,1-2.31,6.17,16.27,16.27,0,0,1-3.43,4.24,27.1,27.1,0,0,1-4.24,3L61.06,79.74H39.92L28.76,52.26H20.82V79.74Z'/>
      </svg>,
    MYR: 
    <svg xmlns='http://www.w3.org/2000/svg' className={props.svgClassName} width='14' height='17' viewBox='0 0 122.88 62.94'>
      <path className={props.className} d='M51.3,62.94,58.76,0H74.17l13,44.22L100.09,0h15.25l7.54,62.94H107l-4-33.87L94.67,62.94H79.26L71,29.72l-3.9,33.22ZM0,62.94V0H26.85a26.91,26.91,0,0,1,7.62,1,14.45,14.45,0,0,1,6,3.43,15.89,15.89,0,0,1,3.94,6.48,32,32,0,0,1,1.4,10.25A29.94,29.94,0,0,1,45.15,28a18,18,0,0,1-1.82,4.87,13,13,0,0,1-2.71,3.35,21.6,21.6,0,0,1-3.35,2.37L48.2,62.94H31.51L22.7,41.25H16.43V62.94ZM16.43,28.21h7.46a6.85,6.85,0,0,0,2.37-.38A4.27,4.27,0,0,0,28,26.6a5.32,5.32,0,0,0,1.05-2.16,12.74,12.74,0,0,0,.34-3.18,12.11,12.11,0,0,0-.34-3.13,4.25,4.25,0,0,0-1.05-2,4.3,4.3,0,0,0-1.74-1.06,7.4,7.4,0,0,0-2.37-.34H16.43V28.21Z'/>
    </svg>,
    NGN: 
    <svg xmlns='http://www.w3.org/2000/svg' className={props.svgClassName} width='8' height='17' viewBox='0 0 115.09 122.88'>
      <path className={props.className} d='M13.42,0H32.1a1.25,1.25,0,0,1,1,.6L58,42.26H83.17v-41A1.23,1.23,0,0,1,84.39,0h17.28a1.23,1.23,0,0,1,1.23,1.23v41h11a1.23,1.23,0,0,1,1.23,1.23V54.55a1.23,1.23,0,0,1-1.23,1.23h-11v9.41h11a1.23,1.23,0,0,1,1.23,1.22V77.48a1.23,1.23,0,0,1-1.23,1.22h-11v43a1.23,1.23,0,0,1-1.23,1.23H84.39a1.25,1.25,0,0,1-1-.6L58,78.7H33.26v43A1.23,1.23,0,0,1,32,122.88H13.42a1.23,1.23,0,0,1-1.23-1.23V78.7h-11A1.23,1.23,0,0,1,0,77.48V66.41a1.23,1.23,0,0,1,1.23-1.22h11V55.78h-11A1.23,1.23,0,0,1,0,54.55V43.49a1.23,1.23,0,0,1,1.23-1.23h11v-41A1.23,1.23,0,0,1,13.42,0ZM33.26,55.78v9.41h17l-4.4-9.41ZM70,65.19H83.17V55.78H65.68L70,65.19ZM83.17,78.7H77.88l5.29,11v-11ZM33.26,32.76v9.5h4.57l-4.57-9.5Z'/>
    </svg>,
    NOK: krSVG,
    SEK: krSVG,
    THB: 
      <svg version='1.0' xmlns='http://www.w3.org/2000/svg'
      width='12' height='14' viewBox='0 0 176.000000 286.000000'
      className={props.svgClassName}
      preserveAspectRatio='xMidYMid meet'>
        <g transform='translate(0.000000,286.000000) scale(0.100000,-0.100000)'
        className={props.className} stroke='none'>
          <path d='M780 2660 l0 -200 -390 0 -390 0 0 -29 c0 -28 1 -28 68 -35 94 -9
          141 -24 173 -55 53 -54 53 -49 57 -841 3 -755 -1 -863 -35 -921 -26 -44 -93
          -72 -185 -77 l-78 -5 0 -28 0 -29 390 0 390 0 0 -215 0 -215 100 0 100 0 0
          214 0 213 123 6 c305 15 506 125 604 333 33 69 38 90 41 169 2 50 -1 117 -8
          148 -38 185 -187 319 -425 383 -44 12 -73 24 -65 27 8 3 40 13 70 22 83 26
          168 74 218 125 154 154 145 461 -18 620 -92 90 -272 159 -462 178 l-73 7 -3
          203 -2 202 -100 0 -100 0 0 -200z m0 -710 l0 -400 -85 0 -85 0 0 380 c0 446
          -6 420 105 420 l65 0 0 -400z m307 348 c117 -43 198 -123 230 -226 24 -77 24
          -228 0 -299 -35 -101 -129 -177 -258 -208 -35 -8 -67 -15 -71 -15 -5 0 -8 176
          -8 390 l0 390 28 -6 c15 -4 51 -15 79 -26z m-307 -1309 l0 -441 -57 4 c-48 4
          -61 10 -85 37 l-28 31 0 405 0 405 85 0 85 0 0 -441z m346 386 c186 -70 284
          -206 284 -395 0 -227 -139 -381 -379 -419 l-51 -8 0 429 0 430 43 -7 c23 -4
          70 -18 103 -30z'/>
        </g>
      </svg>,
    VND: 
      <svg xmlns='http://www.w3.org/2000/svg' className={props.svgClassName} width='12' height='15' viewBox='0 0 78.03 122.88'>
        <path className={props.className} d='M68.59,101.2H50.51V91.12c-.53,0-7,11.77-22.12,11.77q-13.09,0-20.75-9.26T0,68.53q0-16.8,8.6-27.06T31.19,31.19q13.45,0,19,10h.33V22.39H28.45V10.15H50.51V0H68.59V10.15H78V22.39H68.59V101.2ZM50.76,62.08a16.93,16.93,0,0,0-4.42-12,14.53,14.53,0,0,0-11.13-4.8,14.51,14.51,0,0,0-12.35,6.09c-3,4.06-4.51,9.59-4.51,16.64q0,9.69,4.26,15.19c2.85,3.66,6.8,5.49,11.89,5.49a14.42,14.42,0,0,0,11.81-5.76q4.57-5.76,4.45-14.35V62.08Zm17.12,60.8H2.42V111.22H67.88v11.66Z'/>
      </svg>,
    ZAR: 
      <svg xmlns='http://www.w3.org/2000/svg' className={props.svgClassName} width='8' height='17' viewBox='0 0 98.13 122.88'>
        <path className={props.className} d='M26,54.56h8.41q12.35,0,18.21-4.14c3.93-2.72,5.88-7.05,5.88-12.93s-2-10-6-12.45-10.21-3.7-18.62-3.7H26V54.56Zm0,21.16v47.16H0V0H35.8q25,0,37,9.12t12,27.68a32.59,32.59,0,0,1-6,19.3Q72.94,64.54,62,69.33q27.72,41.43,36.12,53.55H69.21L39.9,75.72Z'/>
      </svg>
  };
  return currencies[props.fiat] || null;
};
