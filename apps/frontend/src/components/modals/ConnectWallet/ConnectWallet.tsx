...
const ConnectWallet = () => {
  ...
  const initialConnectValues: ConnectWalletFields = { port: { title: 'Websocket Port', field: 'WS_PORT' }, host: { title: 'CLN Host', field: 'DEVICE_DOMAIN_NAME' }, macaroon: { title: 'Rune', field: 'COMMANDO_RUNE' }, connectUrl: { title: 'Lnmessage URL', field: '' }, clientCert: { title: 'Client Cert', field: '' }, caCert: { title: 'CA Cert', field: '' }, serverCert: { title: 'Server Cert', field: '' } };
  ...
  const copyHandler = (event) => {
    ...
    switch (event.target.id) {
      ...
      case 'CA Cert':
        ...
        break;
      case 'Server Cert':
        textToCopy = appCtx.walletConnect.SERVER_CERT || '';
        break;
      default:
  ...
  const networkChangeHandler = (event) => {
    ...
    switch (event.target.id) {
      case 'LN Message':
        setConnectValues({ port: { title: 'Websocket Port', field: 'WS_PORT' }, host: { title: 'CLN Host', field: 'DEVICE_DOMAIN_NAME' }, macaroon: { title: 'Rune', field: 'COMMANDO_RUNE' }, connectUrl: { title: 'Lnmessage URL', field: '' }, clientCert: { title: 'Client Cert', field: '' }, caCert: { title: 'CA Cert', field: '' }, serverCert: { title: 'Server Cert', field: '' } });
        setConnectUrl('ln-message://' + appCtx.walletConnect.DEVICE_DOMAIN_NAME + ':' + appCtx.walletConnect.WS_PORT + '?rune=' + appCtx.walletConnect.COMMANDO_RUNE);
        break;

      case 'LN Message (Tor)':
        setConnectValues({ port: { title: 'Websocket Port', field: 'WS_PORT' }, host: { title: 'CLN Host', field: 'TOR_DOMAIN_NAME' }, macaroon: { title: 'Rune', field: 'COMMANDO_RUNE' }, connectUrl: { title: 'Lnmessage URL', field: '' }, clientCert: { title: 'Client Cert', field: '' }, caCert: { title: 'CA Cert', field: '' }, serverCert: { title: 'Server Cert', field: '' } });
      ...
      case 'REST':
        setConnectValues({ port: { title: 'REST Port', field: 'REST_PORT' }, host: { title: 'CLN Host', field: 'LOCAL_HOST' }, macaroon: { title: 'Macaroon', field: 'REST_MACAROON' }, connectUrl: { title: 'REST URL', field: '' }, clientCert: { title: 'Client Cert', field: '' }, caCert: { title: 'CA Cert', field: '' }, serverCert: { title: 'Server Cert', field: '' } });
      ...
      case 'REST (Tor)':
        setConnectValues({ port: { title: 'REST Port', field: 'REST_PORT' }, host: { title: 'CLN Host', field: 'TOR_HOST' }, macaroon: { title: 'Macaroon', field: 'REST_MACAROON' }, connectUrl: { title: 'REST URL', field: '' }, clientCert: { title: 'Client Cert', field: '' }, caCert: { title: 'CA Cert', field: '' }, serverCert: { title: 'Server Cert', field: '' } });
      ...
      case 'gRPC':
        setConnectValues({ port: { title: 'gRPC Port', field: 'GRPC_PORT' }, host: { title: 'CLN Host', field: 'DEVICE_DOMAIN_NAME' }, macaroon: { title: 'Client Key', field: 'CLIENT_KEY' }, connectUrl: { title: 'gRPC URL', field: '' }, clientCert: { title: 'Client Cert', field: 'CLIENT_CERT' }, caCert: { title: 'CA Cert', field: 'CA_CERT' }, serverCert: { title: 'Server Cert', field: 'SERVER_CERT' } });
        setConnectUrl('grpc://' + appCtx.walletConnect.DEVICE_DOMAIN_NAME + ':' + appCtx.walletConnect.GRPC_PORT + '?clientkey=' + appCtx.walletConnect.CLIENT_KEY + '&clientCert=' + appCtx.walletConnect.CLIENT_CERT + '&caCert=' + appCtx.walletConnect.CA_CERT + '&serverCert=' + appCtx.walletConnect.SERVER_CERT);
        break;
  
      case 'gRPC (Tor)':
        setConnectValues({ port: { title: 'gRPC Port', field: 'GRPC_PORT' }, host: { title: 'CLN Host', field: 'TOR_DOMAIN_NAME' }, macaroon: { title: 'Client Key', field: 'CLIENT_KEY' }, connectUrl: { title: 'gRPC URL', field: '' }, clientCert: { title: 'Client Cert', field: 'CLIENT_CERT' }, caCert: { title: 'CA Cert', field: 'CA_CERT' }, serverCert: { title: 'Server Cert', field: 'SERVER_CERT' } });
        setConnectUrl('grpc://' + appCtx.walletConnect.TOR_DOMAIN_NAME + ':' + appCtx.walletConnect.GRPC_PORT + '?clientkey=' + appCtx.walletConnect.CLIENT_KEY + '&clientCert=' + appCtx.walletConnect.CLIENT_CERT + '&caCert=' + appCtx.walletConnect.CA_CERT + '&serverCert=' + appCtx.walletConnect.SERVER_CERT);
        break;
    ...
    <>
      <Modal show={appCtx.showModals.connectWalletModal} onHide={closeHandler} centered className='modal-lg' data-testid='connect-wallet'>
        ...
        <Modal.Body className='py-0 px-4'>
            ...
            <Dropdown className='dropdown-network mt-1'>
              ...
              <Dropdown.Menu>
                { networkTypes.map((type) => 
                  <Dropdown.Item as='div' key={type} id={type} onClick={networkChangeHandler}>{type}</Dropdown.Item>
                )}
              </Dropdown.Menu>
            </Dropdown>              
            </Col>
          ...
          <Row className='d-flex align-items-start justify-content-center'>
            <Col xs={selNetwork.includes('gRPC') ? 6 : 12}>
          ...
          { selNetwork.includes('gRPC') ?
            <>
              <Row className='d-flex align-items-start justify-content-center'>
                ...
                <Col xs={6}>
                  <Form.Label className='text-light'>{connectValues.serverCert.title}</Form.Label>
                  <InputGroup className='mb-3'>
                    <Form.Control 
                      onClick={copyHandler}
                      id={connectValues.serverCert.title}
                      value={appCtx.walletConnect[connectValues.serverCert.field]}
                      aria-label={appCtx.walletConnect[connectValues.serverCert.field]}
                      aria-describedby='copy-addon-host'
                      className='form-control-left'
                      readOnly
                    />
                    <InputGroup.Text id={connectValues.serverCert.title} className='form-control-addon form-control-addon-right' onClick={copyHandler}>
                      <CopySVG id={connectValues.serverCert.title} />
                    </InputGroup.Text>
                  </InputGroup>
                </Col>
              </Row>
