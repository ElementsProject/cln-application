...
const NetworkBanner = ({ network }: { network?: string }) => {
  ...
  return (
    <div className={`network-banner bg-${variant} text-center py-1 px-3 mb-2 mx-1 rounded d-flex align-items-center justify-content-center`}>
      <span className='fw-bold text-dark' style={{fontSize: '0.95rem'}}>{label} Network</span>
      <span className='text-dark ms-2' style={{fontSize: '0.95rem'}}>&mdash; Not real bitcoin</span>
    </div>
  );
};
