...
const ChannelOpen = (props) => {
  ...
  if (pubkeyIsValid && amountIsValid && feeRateIsValid) {
    formIsValid = true;
  }
  ...
  const ChannelOpenHandler = event => {
    ...
    CLNService.openChannel(pubkeyValue, +amountValue, (!!showCustomFeeRate ? feeRateValue + 'perkw' : selFeeRate.toLowerCase()), announce)
      .then((response: any) => {
      ...
      <Card className='h-100 d-flex align-items-stretch' data-testid='channel-open-card'>
        <Card.Body className='text-dark d-flex align-items-stretch flex-column pt-4'>
          ...
          <Card.Body className='pb-0 px-1 d-flex flex-column align-items-start justify-content-between'>
            <Row className='d-flex align-items-start justify-content-center'>
              ...
              <Col xs={12} className='d-flex align-items-center mb-3'>
                ...
                <div tabIndex={3} className='switch' data-testid='announce-switch' data-isswitchon={announce} onClick={() => setAnnounce(!announce)}>
              ...
              <AnimatePresence mode="wait">
                {showCustomFeeRate ? (
                  ...
                  >
                    <Col xs={12}>
                      <FeerateRange tabIndex={6} selFeeRate={selFeeRate} selFeeRateChangeHandler={selFeeRateChangeHandler} />
                    </Col>
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <Card.Footer className='d-flex justify-content-center'>
            <button tabIndex={5} data-testid='button-open-channel-submit' type='submit' className='btn-rounded bg-primary' disabled={responseStatus === CallStatus.PENDING}>
              Open Channel
